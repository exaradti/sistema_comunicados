...
export const comunicado_assinatura_schema = z.object({
  comunicado_destinatario_id: z.number().int().positive().optional(),
  token_confirmacao: z.string().min(10).optional(),
  nome_assinante: z.string().min(1),
  assinatura_base64: z.string().min(20)
}).refine((value) => Boolean(value.comunicado_destinatario_id || value.token_confirmacao), {
  message: 'comunicado_destinatario_id ou token_confirmacao é obrigatório'
})
